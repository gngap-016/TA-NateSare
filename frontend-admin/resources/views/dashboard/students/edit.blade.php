  ...
  <div class="card">
    <div class="card-header">
      <div class="row justify-content-between">
        <h4 class="card-title col-10">Edit Student</h4>
        <a href="/admin/users/students" class="col-2 btn btn-secondary">
          <i class="fa-solid fa-arrow-left"></i> Back to Student
        </a>
      </div>
    </div>
    <div class="card-body">
      <form action="/admin/users/students/{{ $user->user_username }}" method="POST" enctype="multipart/form-data">
        @method('PUT')
        @csrf
        <div class="row">
          ...
          <div class="col-md-6">
            <div class="form-group">
              <label for="user_image" class="form-label">Student Profile</label>
          ...
          <div class="col-12">
            <div class="form-group">
              <label for="user_paid_status">Student Paid Status</label>
              <select class="form-select" name="user_paid_status">
                <option value="0" {{ old('user_paid_status', $user->user_paid_status) == 0 ? 'selected' : '' }}>Unsubscribe</option>
                <option value="1" {{ old('user_paid_status', $user->user_paid_status) == 1 ? 'selected' : '' }}>Subscribe</option>
              </select>
            </div>
          </div>
