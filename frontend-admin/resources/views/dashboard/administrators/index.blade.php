  ...
  <div class="card">
    <div class="card-header">
      <div class="row justify-content-between">
        <h4 class="card-title col-10">All Administrators</h4>
        <a href="/admin/users/administrators/create" class="col-2 btn btn-primary">
      ...
      <table class="table table-striped" id="table1">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              <img src="{{ env('SERVER_URL') . 'storage/' . $administrator->user_image }}" class="img-thumbnail" alt="{{ $administrator->user_name }}" style="max-height: 80px">
            </td>
            <td>{{ $administrator->user_username }}</td>
            <td>{{ $administrator->user_name }}</td>
            <td>{{ $administrator->user_email }}</td>
            <td class="text-center">
              <a href="/admin/users/administrators/edit/{{ $administrator->user_username }}" class="btn btn-warning btn-sm">
                <i class="fa-solid fa-pencil"></i>
              </a>
              <button class="btn btn-danger btn-sm" onclick="deleteData('{{ $administrator->user_username }}', '{{ $administrator->user_name }}')">
                <i class="fa-solid fa-trash"></i>
              </button>
              <button class="btn btn-secondary btn-sm" onclick="resetPassword('{{ $administrator->user_username }}', '{{ $administrator->user_name }}')">
...
@section('custom-script')
  ...
  <script>
    function deleteData(parameter, name) {
      ...
      Swal.fire({
        title: 'Are You Sure?',
        html: " Administrator <b>" + name + "</b> will be deleted!",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
          const url = '{{ url("/admin/users/administrators") }}/' + parameter;
          fetch(url, {
            method: "DELETE",
            headers: {
          ...
          .then(result => {
            if(result.status == 'success') {
              Swal.fire(
                'Deleted!',
                'Administrator ' + name + ' has been deleted!',
                'success'
              )
              .then(result => {
                if(result.isConfirmed) {
    ...
    function resetPassword(parameter, name) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          const url = '{{ url("/admin/users/administrators/reset-password") }}/' + parameter;
          fetch(url, {
            method: "PUT",
            headers: {
              'X-CSRF-Token' : document.querySelector('meta[name="csrf-token"]').content
